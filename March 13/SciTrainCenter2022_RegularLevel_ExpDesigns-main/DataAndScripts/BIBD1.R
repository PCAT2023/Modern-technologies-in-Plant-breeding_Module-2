#' ---
#' title:  |
#'   | Case Study  : Balanced Incomplete Block Design - example 1
#' author:
...
#' 
#' The objective is to analyse a small Balanced Incomplete Block 
#' Design, and to see how to generate such a design to implement it 
#' in the field.
#' 
...
BIBD1 <- read.table('BIBD1.txt', sep = ' ', header = TRUE)
str(BIBD1)
BIBD1$blk <- factor(BIBD1$blk)
str(BIBD1)
...
Analysis <- BIB.test(block = BIBD1$blk, 
         trt = BIBD1$trt, 
         y = BIBD1$yield,
         test = c("tukey"),
...
Ana1a <- aov(yield ~ blk + trt, data = BIBD1)
summary(Ana1a)
...
## ** NOT really useful **
Ana1b <- aov(yield ~ trt + blk, data = BIBD1)
summary(Ana1b)
## marginal means ie adjusted means
emmeans(Ana1b, ~ trt)
